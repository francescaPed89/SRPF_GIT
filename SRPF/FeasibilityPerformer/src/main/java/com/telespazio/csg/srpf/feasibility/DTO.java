...

{
    ...
    /*
     * private double [] fourthCorner; private double [] thirdCorner; private
     * double [] secondCorner; private double [] firstCorner;
     */
     ...
     * List of accesses
     */
    protected List<Access> dtoAccessList = new ArrayList<>();
    ...
    /**
     * number of sample in azimuth in case of standard product (in case of
     * spotlight this shall be used) default value 2
     */
    ...
    /**
     *
     * @return the number of sample to evaluate the mean elevation of the scene
     *         at a given azimuth position
     */
    ...
    /**
     * Set the lenght of step (meters) to evaluate the mean elevation at near
     * off nadir
     *
    ...
    /**
     *
     * @return the number of step (half interval) to evaluate the mean elevation
     *         at near off nadir
     */
    ...
    /**
     * Set the number of step (half interval) to evaluate the mean elevation at
     * near off nadir
     *
    ...
    /**
     * Set the lenght of the step (meters) to evaluate the mean elevation at
     * near off nadir
     *
    ...
    /**
     *
     * @return the number of step to evaluate the mean elevation at sub
     *         satellite point
     */
    ...
    /**
     * Set the number of step to evaluate the mean elevation at sub satellite
     * point
     *
    ...
    public DTO(Element dtoel) throws XPathExpressionException, GridException
    {
         ...
         */
        String satellite = XMLUtils.getChildElementText(dtoel, FeasibilityConstants.SatelliteTagName, FeasibilityConstants.SatelliteTagNameNS);
        String startTime = XMLUtils.getChildElementText(dtoel, FeasibilityConstants.timeStartTagName, FeasibilityConstants.timeStartTagNameNS);
        String timeStop = XMLUtils.getChildElementText(dtoel, FeasibilityConstants.timeStopTagName, FeasibilityConstants.timeStopTagNameNS);
        ...
        this.missionName = XMLUtils.getChildElementText(dtoel, FeasibilityConstants.MissionTagName, FeasibilityConstants.MissionTagNameNS);

        /**
         * Building geometric info
         */
        String posList = XMLUtils.getChildElementText(dtoel, FeasibilityConstants.PosListTagName, FeasibilityConstants.PosListTagNameNS);
         ...
         * side info
         */
        String idString = XMLUtils.getChildElementText(dtoel, FeasibilityConstants.DTOIdTagName, FeasibilityConstants.DTOIdTagNameNS);
        // this.id=Integer.parseInt(idString);

        this.id = idString;

        String orbitDirectionAsString = XMLUtils.getChildElementText(dtoel, FeasibilityConstants.orbitDirectionTagName, FeasibilityConstants.orbitDirectionTagNameNS);
        ...
        String lookSideString = XMLUtils.getChildElementText(dtoel, FeasibilityConstants.LookSideTagName, FeasibilityConstants.LookSideTagNameNS);
         ...
         */
        this.polarization = XMLUtils.getChildElementText(dtoel, FeasibilityConstants.PolarizationTagName, FeasibilityConstants.PolarizationTagNameNS);
        this.sarBeamName = XMLUtils.getChildElementText(dtoel, FeasibilityConstants.BeamIdTagName, FeasibilityConstants.BeamIdTagNameNS);
        this.sarMode = XMLUtils.getChildElementText(dtoel, FeasibilityConstants.SensorModeTagName, FeasibilityConstants.SensorModeTagNameNS);

        this.sparcInfo = XMLUtils.getChildElementText(dtoel, FeasibilityConstants.SPARCInfoTagName, FeasibilityConstants.SPARCInfoTagNameNS);
    ...
    /**
     * IF decorrelation time == 8 days or decorrelation time == 16 days, we
     ...
     *         decorrelation time == 8 days or decorrelation time == 16 days,
     *         //we force to use interferometric DTO in future resprect to the
     *         base DTO boolean retval=true;
     ...
     */
    public DTO getInterferometricDTO(final Satellite interferometricSat, double decorrelationTime, double decorrelationTolerance, double stopValidityTime)
    {
        ...
        for (Access a : interferometricSat.getAccessList())
        {
            ...
            // decorrelationTime))
            if ((difftime1 < decorrelationTolerance) && (side == a.getLookSide()) && beamName.equals(a.getBeamId()) && (orbitDir == a.getOrbitDirection()) && (basePointId == pointId) && (epoch < a.getAccessTime()))
            {
                /**
                 * current access satisfy decorrelation constraint has the same
                 * beam orbit and side of the base access so it coul be added to
                 * the interferometric access list
                 */
                ...
                {
                    this.tracer.warning(EventType.SOFTWARE_EVENT, ProbableCause.INFORMATION_INFO, "Error in evaluating Epochs for interferometric DTO. Use linked values if CSK otherwhise discard");
                    /**
                     * Using the satellite PVT ofe base satellute for the
                     * interferometric one
                     */
    ...
    /**
     * If the DTO fall inside a pw this method return the relevan paw otherwise
     * null
     *
     ...
     *
     */
    public DTO(String id, double startTime, double stopTime, boolean isOdrefBased, double[] lastAccessNearFarCorner, double[] lastAccessFarCorner, double[] firstFarCorner, double[] firstAccessNearCorner, Satellite sat, long trakNumber, long orbitId)
    {
        /**
         * Assigning
         *
    ...
    /**
     * Check the strip duration and cuts it to maximal duration in case and
     * delete the not available access
     *
     * @param s
     */
    ...
    {
        /**
         * check if the mean elevation has already evakuated and if the
         * accesslist is empty
         */
    ...
    private void buildDTO(Strip s)
    {
        ...
        if (duration < minimalDuration)
        {
            ...
            catch (FeasibilityException e)
            {
                ...
                this.tracer.warning(EventType.APPLICATION_EVENT, ProbableCause.CONFIGURATION_OR_CUSTOMIZATION_ERROR, e.getMessage());
            }
    ...
    /**
     * return true if the DTO is under an undeferreble paw, so not usable More
     * over it also check the flag for deferreble flag
     *
    ...
    public boolean checkForPassPlan()
    {
        ...
        /*
         * tracer.
         * debug("==================Perfoming check for passrhrough on DTO " +
        ...
        {
            this.tracer.warning(EventType.APPLICATION_EVENT, ProbableCause.CONFIGURATION_OR_CUSTOMIZATION_ERROR, "The evaluated DWL Duration ends before the stop of DTO acquisition. In passthrough check the stop time of DTO will be used");
    ...
    /**
     * Return near and far corner given the access and satellite position and
     * velocity and off nadir angle. Used directly in SPARCManager. The opening
     * of beam is specified
     *
     * @param satPos
     * @param satVel
     * @param nearOffNadir
     ...
     */
    public double[][] evaluateLLHcorners(Vector3D satPos, Vector3D satVel, double nearOffNadirDeg, double farOffNadirDeg)

    {
        ...
        Vector3D pointingVersor = e[1].scalarMultiply(Math.sin(nearOffNadirP)).add(e[2].scalarMultiply(Math.cos(nearOffNadirP)));
    ...
    private double getPointingModule(double[] ecefPos, double[] pointingVersor, double h)
    {
        ...
        double a = (((pointingVersor[0] * pointingVersor[0]) + (pointingVersor[1] * pointingVersor[1])) * d * d) + (pointingVersor[2] * pointingVersor[2]);
        double b = 2 * ((d * d * ((pointingVersor[0] * ecefPos[0]) + (pointingVersor[1] * ecefPos[1]))) + (pointingVersor[2] * ecefPos[2]));
        ...
        double c = ((d * d * ((ecefPos[0] * ecefPos[0]) + (ecefPos[1] * ecefPos[1]))) + (ecefPos[2] * ecefPos[2])) - (eqRadius2 * d2);
     ...
     *
     *
     * double [] corner1Ecef = ReferenceFrameUtils.llh2ecef(this.corners[0],
     * true); double [] corner2Ecef =
     * ReferenceFrameUtils.llh2ecef(this.corners[1], true); double []
     * corner3Ecef = ReferenceFrameUtils.llh2ecef(this.corners[2], true); double
     * [] corner4Ecef = ReferenceFrameUtils.llh2ecef(this.corners[3], true);
     *
     ...
     *
     * double [] llhCenter =
     * ReferenceFrameUtils.ecef2llh(geometricCenterECEF,true);
     *
     ...
     * ; distance=calc.getOrthodromicDistance()/1000.0; } catch(Exception e) {
     * //TODO loggare double [] targetCenterEcef =
     * ReferenceFrameUtils.llh2ecef(llhTargetCenter, true); Vector3D geoCenter =
     * new Vector3D(geometricCenterECEF); Vector3D targetCenter = new
    ...
    public double getTargetDistance(double[] llhTargetCenter)
    {
         ...
         * true); double [] corner2Ecef =
         * ReferenceFrameUtils.llh2ecef(this.corners[1], true); double []
         * corner3Ecef = ReferenceFrameUtils.llh2ecef(this.corners[2], true);
         * double [] corner4Ecef = ReferenceFrameUtils.llh2ecef(this.corners[3],
         * true);
         */
        /**
         ...
         * secondMiddlePointLLH =
         * ReferenceFrameUtils.ecef2llh(secondMiddlePoint, true);
         */
        ...
        /**
         * We apply Herone formula to evaluate the triangle having target and
         * the two middle point as vertexes The distance is the height respect
         * the base formed by the two middle points
         */
        ...
        double halfPerimeter = FeasibilityConstants.half * (base + firstMiddleToTargetDistance + secondMiddleToTargetDistance);

        /**
         * Area Herone
         */
        double A2 = halfPerimeter * (halfPerimeter - base) * (halfPerimeter - firstMiddleToTargetDistance) * (halfPerimeter - secondMiddleToTargetDistance);
    ...
    public int getLookSide()
    {
        ...
        {
            /**
             * Look side could be only 1 or 2 so < 0 is a value not allowed so
             * it maust be evaluated
             */
    ...
    public int getOrditDirection()
    {
        ...
        /**
         * Look side could be only 1 or 2 so < 0 is a value not allowed so it
         * must be evaluated
         */
    ...
    public String getBeamId()
    {
        ...
        return this.dtoAccessList.get(0).getBeamId();
    }// end method
    ...
    {
        // return this.strip.getBeam();
        return this.dtoAccessList.get(0).getBeam();
    }// end method
    ...
    public String toString()
    {
        ...
        String retString =  "DTO [id=" + id + ", startTime=" + DateUtils.fromCSKDateToDateTime(startTime) + ", stopTime=" + DateUtils.fromCSKDateToDateTime(stopTime) + ", corners=" + Arrays.toString(corners) +"";  
        for(int i = 0; i<4; i++)
        {
            for(int j = 0; j<3; j++)
            {
