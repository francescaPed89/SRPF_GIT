/**
*
* MODULE FILE NAME:	ODSTPHandler.java
*
...
public class ODSTPHandler implements CM_INotification
{
    ...
    /*
     * Creates a new handler for ODSTP requests
     *
     ...
     */
    // public ODSTPHandler (final CM_Services cmServ, final Properties
    // cmIngestorConf)
    public ODSTPHandler(final CM_Services cmServ, final TraceManager tracer)
    {
        this.cmServ = cmServ;
     ...
     *
     * @param msgID
     *            ID of the ODSTP message to fetch from CM
     */
    public void NewMessageAvailable(final int msgID)
    {
        ...
        int timeout = Integer.parseInt(this.cmIngestorConf.getProperty(CMConstants.CM_ODSTP_TIMEOUT, CMConstants.CM_DEFAULT_TIMEOUT)); // Timeout
                                                                                                                                       // for
                                                                                                                                       // fetching
                                                                                                                                       // ODSTP
                                                                                                                                       // requests
                                                                                                                                       // from
                                                                                                                                       // CM
        String attachmentPath = null; // Path of the attachment of the ODSTP
                                      // message
        String ODSTPFilePath = null; // The path of the moved file

        try
        {
            CM_Message request = this.cmServ.GetMessage(filter, timeout); // ODSTP
                                                                          // message
                                                                          // from
                                                                          // CM
            ...
            // msgID);
            this.tracer.log("It stated: " + new String(request.GetBody()));
            this.tracer.log("request attachment : " + request.GetAttachments());
        ...
        finally
        {
            ...
            /*
             * if (ODSTPFilePath != null) { cleanUp(new
             * File(ODSTPFilePath).getParent()); } // end if
     ...
     *            Path of the attachment of the ODSTP message
     * @param msgID
     *            ID of the ODSTP message fetched from CM
     *
     * @throws IOException
    ...
    private String moveAttachment(final String attachmentPath, final String msgID) throws IOException
    {
        ...
        // String workingDir =
        // this.cmIngestorConf.getProperty(CMConstants.CM_ODSTP_WORKING_DIR) +
        // "/" + msgID + "_" + currentDateTime; // Working directory where the
        // attachments are moved
        String workingDir = this.cmIngestorConf.getProperty(CMConstants.CM_ODSTP_WORKING_DIR);

        if (workingDir == null)
        {
            String msg = "No working dir configured for ODSTP";
     ...
     */
    private void performODSTP(final String ODSTPFilePath) throws MalformedURLException, SrpfBackendException_Exception
    {
        ...
        WSController wscProxy = wscService.getWSController(); // WSController
                                                              // proxy used to
                                                              // invoke methods
        wscProxy.manageODSTP(ODSTPFilePath); // Result of the ODSTP request
    } // end method
