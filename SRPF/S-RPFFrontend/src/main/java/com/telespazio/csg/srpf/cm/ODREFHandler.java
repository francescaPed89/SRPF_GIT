/**
*
* MODULE FILE NAME:	ODREFHandler.java
*
...
public class ODREFHandler implements CM_INotification
{
    ...
    /*
     * Creates a new handler for ODREF requests
     *
     ...
     */
    // public ODREFHandler (final CM_Services cmServ, final Properties
    // cmIngestorConf)
    public ODREFHandler(final CM_Services cmServ, final TraceManager tracer)
    {
        this.cmServ = cmServ;
     ...
     *
     * @param msgID
     *            ID of the ODREF message to fetch from CM
     */
    public void NewMessageAvailable(final int msgID)
    {
        ...
        int timeout = Integer.parseInt(this.cmIngestorConf.getProperty(CMConstants.CM_ODREF_TIMEOUT, CMConstants.CM_DEFAULT_TIMEOUT)); // Timeout
                                                                                                                                       // for
                                                                                                                                       // fetching
                                                                                                                                       // ODREF
                                                                                                                                       // requests
                                                                                                                                       // from
                                                                                                                                       // CM
        String attachmentPath = null; // Path of the attachment of the ODREF
                                      // message
        String ODREFFilePath = null; // The path of the moved file

        try
        {
            CM_Message request = this.cmServ.GetMessage(filter, timeout); // ODREF
                                                                          // message
                                                                          // from
                                                                          // CM

            this.tracer.log("ODREFReceived message with ID: " + msgID);
            ...
            // msgID);
            this.tracer.log("ODREF It stated: " + new String(request.GetBody()));
            this.tracer.log("ODREF request attachment : " + request.GetAttachments());
            ...
            if (attachment != null)
            {
                ...
                this.tracer.log("ODREF attachmentPath : " + attachmentPath);

                ODREFFilePath = moveAttachment(attachmentPath, new Integer(msgID).toString()); // The
                                                                                               // path
                                                                                               // of
                                                                                               // the
                                                                                               // moved
                                                                                               // file
                this.tracer.log("ODREF Attachment: " + attachmentPath + " moved to working dir " + ODREFFilePath);
                performODREF(ODREFFilePath);
                this.tracer.log("ODREF Operation completed successfully");
                // Sending report to DSMT
                DSMTReporter.getInstance().sendReport(DSMTReporterConstants.SecurityEnum.OPERATIONAL, DSMTReporterConstants.ConfidentialFlag.NC, DSMTReporterConstants.ConfidentialLevel.UNCLASSIFIED, DSMTReporterConstants.Severity.NOSEVERITY, DSMTReporterConstants.ReportType.INFORMATION, DSMTReporterConstants.ODREFReceived, "Received ODREF FILE: " + Paths.get(attachmentPath).getFileName().toString(), this.cmServ);
            } // end if
            else
            {
        ...
        {
            this.tracer.log("ODREF CM_Exception: " + cme.getMessage());
        ...
        {
            this.tracer.log("ODREF IOException: " + ioe.getMessage());
        ...
        finally
        {
            ...
            /*
             * if (ODREFFilePath != null) { cleanUp(new
             * File(ODREFFilePath).getParent()); } // end if
     ...
     *            Path of the attachment of the ODREF message
     * @param msgID
     *            ID of the ODREF message fetched from CM
     *
     * @throws IOException
    ...
    private String moveAttachment(final String attachmentPath, final String msgID) throws IOException
    {
        ...
        // String workingDir =
        // this.cmIngestorConf.getProperty(CMConstants.CM_ODREF_WORKING_DIR) +
        // "/" + msgID + "_" + currentDateTime; // Working directory where the
        // attachments are moved

        String workingDir = this.cmIngestorConf.getProperty(CMConstants.CM_ODREF_WORKING_DIR);

        if (workingDir == null)
        {
            String msg = "No working dir configured for ODREF";
     ...
     */
    private void performODREF(final String ODREFFilePath) throws MalformedURLException, SrpfBackendException_Exception
    {
        ...
        WSController wscProxy = wscService.getWSController(); // WSController
                                                              // proxy used to
                                                              // invoke methods
        wscProxy.manageODREF(ODREFFilePath); // Result of the ODREF request
    } // end method
