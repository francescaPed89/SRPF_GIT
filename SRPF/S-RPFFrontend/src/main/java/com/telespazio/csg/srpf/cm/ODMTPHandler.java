/**
*
* MODULE FILE NAME:	ODMTPHandler.java
*
...
/**
 * Handles ODMTP requests by CM
 * 
 * @author Girolamo Castaldo
 * @version 2.0
 */

public class ODMTPHandler implements CM_INotification
{
    ...
    /*
     * Creates a new handler for ODMTP requests
     *
     ...
     */
    // public ODMTPHandler (final CM_Services cmServ, final Properties
    // cmIngestorConf, final TraceManager tracer, final EventManager eventMgr)
    public ODMTPHandler(final CM_Services cmServ, final TraceManager tracer)
    {
        this.cmServ = cmServ;
     ...
     *
     * @param msgID
     *            ID of the ODMTP message to fetch from CM
     */
    public void NewMessageAvailable(final int msgID)
    {
        ...
        int timeout = Integer.parseInt(this.cmIngestorConf.getProperty(CMConstants.CM_ODMTP_TIMEOUT, CMConstants.CM_DEFAULT_TIMEOUT)); // Timeout
                                                                                                                                       // for
                                                                                                                                       // fetching
                                                                                                                                       // ODMTP
                                                                                                                                       // requests
                                                                                                                                       // from
                                                                                                                                       // CM
        String attachmentPath = null; // Path of the attachment of the ODMTP
                                      // message
        String ODMTPFilePath = null; // The path of the moved file

        try
        {
            CM_Message request = this.cmServ.GetMessage(filter, timeout); // ODMTP
                                                                          // message
                                                                          // from
                                                                          // CM
            ...
            if (attachment != null)
            {
                ...
                ODMTPFilePath = moveAttachment(attachmentPath, new Integer(msgID).toString()); // The
                                                                                               // path
                                                                                               // of
                                                                                               // the
                                                                                               // moved
        ...
        finally
        {
            ...
            /*
             * if (ODMTPFilePath != null) { cleanUp(new
             * File(ODMTPFilePath).getParent()); } // end if
     ...
     *            Path of the attachment of the ODMTP message
     * @param msgID
     *            ID of the ODMTP message fetched from CM
     *
     * @throws IOException
    ...
    private String moveAttachment(final String attachmentPath, final String msgID) throws IOException
    {
        ...
        // String workingDir =
        // this.cmIngestorConf.getProperty(CMConstants.CM_ODMTP_WORKING_DIR) +
        // "/" + msgID + "_" + currentDateTime; // Working directory where the
        // attachments are moved
        String workingDir = this.cmIngestorConf.getProperty(CMConstants.CM_ODMTP_WORKING_DIR);

        if (workingDir == null)
        {
            String msg = "No working dir configured for ODMTP";
     ...
     */
    private void performODMTP(final String ODMTPFilePath) throws MalformedURLException, SrpfBackendException_Exception
    {
        ...
        WSController wscProxy = wscService.getWSController(); // WSController
                                                              // proxy used to
                                                              // invoke methods
        wscProxy.manageODMTP(ODMTPFilePath); // Result of the ODMTP request
    } // end method
