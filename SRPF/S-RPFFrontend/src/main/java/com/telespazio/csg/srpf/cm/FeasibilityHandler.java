/**
*
* MODULE FILE NAME:	FeasibilityHandler.java
*
* MODULE TYPE:		Class definition
*
* FUNCTION:			Feasinbility Ingestion
*
* PURPOSE:			Feasibility Ingestion
*
...
/**
 * Handles feasibility requests by CM
 * 
 * @author Girolamo Castaldo
 * @version 1.0
 */
public class FeasibilityHandler implements CM_INotification
{
    ...
    public FeasibilityHandler(final CM_Services cmServ, final TraceManager tracer)
    {
        this.cmServ = cmServ;
    ...
    public void NewMessageAvailable(final int msgID)
    {
        ...
        int timeout = Integer.parseInt(this.cmIngestorConf.getProperty(CMConstants.CM_FEASIBILITY_TIMEOUT, CMConstants.CM_DEFAULT_TIMEOUT)); // Timeout
                                                                                                                                             // for
                                                                                                                                             // fetching
                                                                                                                                             // feasibility
                                                                                                                                             // requests
        ...
        try
        {
            ...
            if (attachment != null)
            {
                ...
                result = performFeasibility(feasibilityFilePath);
            } // end if
            else
            {
    ...
    private String moveAttachment(final String attachmentPath, final String msgID) throws IOException
    {
        ...
        // String workingDir =
        // this.cmIngestorConf.getProperty(CMConstants.CM_FEASIBILITY_WORKING_DIR)
        // + "/" + msgID + "_" + currentDateTime; // Working directory where the
        // attachments are moved

        String workingDir = this.cmIngestorConf.getProperty(CMConstants.CM_FEASIBILITY_WORKING_DIR);

        if (workingDir == null)
        {
            String msg = "No working dir configured for FeasibilityRequest";
     ...
     */
    private String performFeasibility(final String feasibilityFilePath) throws IOException, MalformedURLException, SrpfBackendException_Exception
    {
        ...
        String result = wscProxy.performFeasibility(feasibilityFilePath); // Result
                                                                          // of
                                                                          // the
                                                                          // feasibility
                                                                          // request
