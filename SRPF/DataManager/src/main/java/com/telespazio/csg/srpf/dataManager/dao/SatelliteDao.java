...
public class SatelliteDao extends GenericDAO
{
    ...
    public SatelliteBean getDatiSatellite(String SatelliteName) throws Exception
    {
        ...
        String query = "SELECT ID_SATELLITE, SATELLITE_NAME, MISSION, IS_ENABLED  , ID_ALLOWED_LOOK_SIDE,TRACK_OFFSET,  MISSION_NAME  from SATELLITE s, MISSION m where   SATELLITE_NAME = " + "'" + SatelliteName + "'" + " and s.MISSION=m.ID_MISSION ";
        ...
        {
            // log
            logger.error(EventType.SOFTWARE_EVENT, "Execution error of the query by prepared statement: ", e.getMessage());
            // rethow
            throw e;
        } // end catch
        return p;
    ...
    public Map<SatelliteBean, List<BeamBean>> getBeamsSatellite(String missionNameOrFirstSensorMode, String sensorModeName, boolean isCombined) throws Exception
    {
        ...
        SatelliteDao.logger.debug("Inside method getBeamsSatellite Map<SatelliteBean, List<BeamBean>> of satelliteDao ");

        String querona;
        if (!isCombined)
        {
            //SatelliteDao.logger.debug("getBeamsSatellite : not a combined request, input parameters are : " + missionNameOrFirstSensorMode + " as missionName and " + sensorModeName + " as sensor mode name");
            ...
            // request
        	  querona ="SELECT SATELLITE.SATELLITE_NAME, MISSION.MISSION_NAME, SATELLITE.IS_ENABLED, SATELLITE.ID_ALLOWED_LOOK_SIDE, SATELLITE.ID_SATELLITE ,MISSION.ID_MISSION,SATELLITE.TRACK_OFFSET, SENSOR_MODE.ID_SENSOR_MODE, BEAM.ID_BEAM, BEAM.BEAM_NAME, BEAM.IS_ENABLED, BEAM.NEAR_OFF_NADIR, BEAM.FAR_OFF_NADIR, BEAM.SW_DIM1, BEAM.SW_DIM2, SENSOR_MODE.IS_SPOT_LIGHT, SENSOR_MODE.SENSOR_MODE_NAME, BEAM.DTO_MIN_DURATION, BEAM.DTO_MAX_DURATION, BEAM.RES_TIME, BEAM.DTO_DURATION_SQUARED from SATELLITE," + 
        	  		...
        	  		" SENSOR_MODE," + 
        	  		" BEAM " + 
        	  		" WHERE SATELLITE.MISSION = MISSION.ID_MISSION" + 
        	  		" AND MISSION.MISSION_NAME= " + "'" + missionNameOrFirstSensorMode+ "'" + " AND " + 
        	  		" BEAM.SENSOR_MODE=SENSOR_MODE.ID_SENSOR_MODE AND SENSOR_MODE.SENSOR_MODE_NAME=" + "'" + sensorModeName + "'" +" AND BEAM.ID_BEAM=SAT_BEAM_ASSOCIATION.BEAM AND SATELLITE.ID_SATELLITE=SAT_BEAM_ASSOCIATION.SATELLITE order by SATELLITE.SATELLITE_NAME, BEAM.BEAM_NAME " ; 
        ...
        /*
         * FRANCESCA : questa parte di codice dell'else non viene mai
         * utilizzata, rimuovibile
         */
        else
        {

            SatelliteDao.logger.debug("getBeamsSatellite : combined request, input parameters are : " + missionNameOrFirstSensorMode + " as first sensor mode name and " + sensorModeName + " as second sensor mode name");

            // query
            // used for single mission
            querona = "SELECT SAT_NAME, MISSION_NAME, ENABLED,  ALLOWED_SIDE, SAT_ID, MISSION_ID, TOFF, SMODE_ID," + " BEAM_ID, BNAME, IS_ENABLED_BEAM,NEAR_OFF_NADIR,FAR_OFF_NADIR," + " SW_DIM1, SW_DIM2, IS_SPOT_LIGHT, SENSOR_MODE_NAME, DTO_MIN_DURATION, DTO_MAX_DURATION, REST_TIME " + " from (SELECT SATELLITE.SATELLITE_NAME as SAT_NAME, MISSION.MISSION_NAME as MISSION_NAME,SATELLITE.IS_ENABLED as ENABLED," + " SATELLITE.ID_ALLOWED_LOOK_SIDE AS ALLOWED_SIDE, SATELLITE.ID_SATELLITE as SAT_ID, MISSION.ID_MISSION as MISSION_ID, SATELLITE.TRACK_OFFSET as TOFF " + "from SATELLITE INNER join MISSION on SATELLITE.MISSION = MISSION.ID_MISSION  " + ") inner join" + "( SELECT SMODE_ID, BEAM_ID,BNAME," + " SAT_BEAM_ASSOCIATION.IS_ENABLED as IS_ENABLED_BEAM, BEAM.NEAR_OFF_NADIR as NEAR_OFF_NADIR, " + "BEAM.FAR_OFF_NADIR as FAR_OFF_NADIR, " + " SW_DIM1, SW_DIM2, IS_SPOT_LIGHT, SENSOR_MODE_NAME, DTO_MIN_DURATION,DTO_MAX_DURATION, REST_TIME, "
                    + " SAT_BEAM_ASSOCIATION.SATELLITE as ASSOCIATED_SAT " + "from( SELECT " + " "
                            + "SENSOR_MODE.ID_SENSOR_MODE as SMODE_ID, "
                            + "BEAM.ID_BEAM as BEAM_ID, "
                            + "BEAM.BEAM_NAME as BNAME, "
                            + "BEAM.SW_DIM1 as SW_DIM1," + " "
                                    + "BEAM.SW_DIM2 as SW_DIM2, "
                                    + "SENSOR_MODE.IS_SPOT_LIGHT as IS_SPOT_LIGHT, "
                                    + "SENSOR_MODE.SENSOR_MODE_NAME as SENSOR_MODE_NAME," + " "
                                            + "BEAM.DTO_MIN_DURATION as DTO_MIN_DURATION, "
                                            + "BEAM.DTO_MAX_DURATION as DTO_MAX_DURATION, " + " "
                                                    + "BEAM.RES_TIME as REST_TIME , "
                                                    + "BEAM.DTO_DURATION_SQUARED AS DTO_DURATION_SQUARED " 
                                                    + "from BEAM INNER join SENSOR_MODE on BEAM.SENSOR_MODE=SENSOR_MODE.ID_SENSOR_MODE " + " where SENSOR_MODE.SENSOR_MODE_NAME=" + "'" + missionNameOrFirstSensorMode + "' OR SENSOR_MODE.SENSOR_MODE_NAME=" + "'" + sensorModeName + "'" + " ) Inner join SAT_BEAM_ASSOCIATION on BEAM_ID=SAT_BEAM_ASSOCIATION.BEAM ) " + " on SAT_ID=ASSOCIATED_SAT order by SAT_NAME, BNAME";
        ...
        try
        {
            ...
            // ManagerLogger.logDebug(this, query);
            PreparedStatement st = this.con.prepareStatement(querona);
            ResultSet rs = null;
            ...
            try
            {
                ...
                while (rs.next())
                {
                    ...
                    if (!currentSatName.equals(newSatName))
                    { // filling satellite bean
        ...
        {
            // log
            SatelliteDao.logger.error(EventType.SOFTWARE_EVENT, "Execution error of the query by prepared statement: "+querona, e.getMessage());
    ...
    {
        // Mega query
        String query = "SELECT SENSOR_MODE as \"Sensor Mode\", "
                + "s.SATELLITE_NAME AS \"Satellite Name\", "
                + "b.ID_BEAM AS \"ID BEAM\", "
                + "b.BEAM_NAME AS \"Beam Name\"," 
                + "b.IS_ENABLED AS \"Is Enabled\", "
                + "b.NEAR_OFF_NADIR AS \"Near Off Nadir\" ,"
                + "b.FAR_OFF_NADIR AS\" Far Off Nadir\"" + " , "
                + "b.SW_DIM1 AS\" Sw Dim1\", "
                + "b.SW_DIM2 AS\" Sw Dim2\", "
                + "sm.IS_SPOT_LIGHT AS\"Spot Light\", "
                + "sm.SENSOR_MODE_NAME AS\" Sensor Name\"  " + " , "
                + "b.DTO_MIN_DURATION AS\" Dto Min\", "
                + "b.DTO_MAX_DURATION AS\" Dto Max\" , "
                + "b.RES_TIME AS\" Res Time\" , " 
                + "b.DTO_DURATION_SQUARED AS\" Duration Squared\" " + " from "
                + "BEAM b,"
                + "SAT_BEAM_ASSOCIATION sb, "
                + "satellite s, "
                + "SENSOR_MODE sm  "
                + ""
                + "where b.ID_BEAM(+)=sb.BEAM  and sb.SATELLITE=s.ID_SATELLITE " + " and b.SENSOR_MODE=sm.ID_SENSOR_MODE and s.ID_SATELLITE= " + idSatellite + " and b.SENSOR_MODE= " + idSensorMode;
        //
        // log
        //
    ...
    public ArrayList<SatelliteBean> getSatellitesPerSensorMode(int idSensorMode, int idMission) throws Exception
    {
         ...
         * + " and b.SENSOR_MODE= " + idSensorMode +
         * " and m.ID_MISSION= s.MISSION" + " and  m.ID_MISSION= " + idMission +
         * " order by s.SATELLITE_NAME";
         */
        String query = "SELECT  distinct(s.SATELLITE_NAME), "
                + "MISSION_NAME,  "
                + "s.IS_ENABLED, "
                + "s.ID_ALLOWED_LOOK_SIDE, "
                + "s.ID_SATELLITE, "
                + "s.TRACK_OFFSET  "
                + ""
                + "from BEAM b,"
                ...
                + "MISSION m  "
                + ""
                + "where b.ID_BEAM(+)=sb.BEAM  and "
                + "sb.SATELLITE=s.ID_SATELLITE  " + " and "
                        + "b.SENSOR_MODE= " + idSensorMode + " and "
                                + "m.ID_MISSION= s.MISSION" + " and  "
                                        + "m.ID_MISSION= " + idMission + " order by s.SATELLITE_NAME";
    ...
    {
        // query
        String query = "SELECT SATELLITE.SATELLITE_NAME , MISSION.MISSION_NAME, SATELLITE.IS_ENABLED, " + "SATELLITE.ID_ALLOWED_LOOK_SIDE, SATELLITE.ID_SATELLITE, SATELLITE.TRACK_OFFSET " + " from  SATELLITE join MISSION on SATELLITE.MISSION=MISSION.ID_MISSION";
    ...
    public ArrayList<SatelliteBean> getSatellitesPerSensorModeConfiguration(int idSensorMode, int idMission) throws Exception
    {
         ...
         * " order by s.SATELLITE_NAME";
         */
        String query = "SELECT  distinct(s.SATELLITE_NAME) , MISSION_NAME ,  s.IS_ENABLED , s.ID_ALLOWED_LOOK_SIDE , s.ID_SATELLITE,s.TRACK_OFFSET  " + " from BEAM b,SAT_BEAM_ASSOCIATION sb, SATELLITE s, MISSION m  where b.ID_BEAM(+)=sb.BEAM  and sb.SATELLITE=s.ID_SATELLITE  " + " and b.SENSOR_MODE= " + idSensorMode + " and m.ID_MISSION= s.MISSION" + " and  m.ID_MISSION= " + idMission + " order by s.SATELLITE_NAME";

        // ManagerLogger.logDebug(this, query);
        //this.tm.debug("Inside method getSatellitesPerSensorModeConfiguration " + query);
        PreparedStatement st = this.con.prepareStatement(query);
        ResultSet rs = null;
        ArrayList<SatelliteBean> satsList = new ArrayList<>();
    ...
    public boolean existSatelliteBeam(int idSatellite) throws Exception
    {
        ...
        PreparedStatement st = this.con.prepareStatement(sql.toString());
        ResultSet rs = null;
        // try
        try
        {
    ...
    {
        PreparedStatement pst = this.con.prepareStatement(inputQuery);
    ...
    public ArrayList<BeamBean> getBeamsExportConfiguration(int idSensorMode, int idSatellite) throws Exception
    {
        ...
        String query = "SELECT SENSOR_MODE as \"Sensor Mode\", "
                ...
                + "b.BEAM_NAME AS \"Beam Name\"" + ",  "
                + "b.IS_ENABLED AS \"Is Enabled\", "
                + "b.NEAR_OFF_NADIR AS \"Near Off Nadir Beam\" ,"
                + "b.FAR_OFF_NADIR AS\" Far Off Nadir Beam\" " + ", "
                + "b.SW_DIM1 AS\" Sw Dim1\", "
                + "b.SW_DIM2 AS\" Sw Dim2\", "
                + "sm.IS_SPOT_LIGHT AS\"Spot Light\"  " + " , "
                + "b.DTO_MIN_DURATION AS\" Dto Min\", "
                ...
                + "satellite s, "
                + "SENSOR_MODE sm  "
                + "where b.ID_BEAM(+)=sb.BEAM  " + " and b.SENSOR_MODE=sm.ID_SENSOR_MODE" + " and sb.SATELLITE=s.ID_SATELLITE and s.ID_SATELLITE= " + idSatellite + " and b.SENSOR_MODE= " + idSensorMode;
    ...
    public ArrayList<String> getFileNameForOrbitData(String satellite) throws Exception
    {
        ...
        // query string
        String queryString = "SELECT SATELLITE.SATELLITE_NAME,OBDATA_FILES.ODSTP, OBDATA_FILES.ODMTP, OBDATA_FILES.ODNOM,OBDATA_FILES.ODREF " + "from SATELLITE  join OBDATA_FILES on  SATELLITE.ID_SATELLITE = OBDATA_FILES.ID_SATELLITE " + "and SATELLITE.SATELLITE_NAME = '" + satellite + "'";
        this.con.setAutoCommit(false);
        // stm
        PreparedStatement st = this.con.prepareStatement(queryString);
    ...
    public Map<String, ArrayList<String>> getFileNameForOrbitData() throws Exception
    {
        ...
        // query string
        String queryString = "SELECT SATELLITE.SATELLITE_NAME,OBDATA_FILES.ODSTP, OBDATA_FILES.ODMTP, OBDATA_FILES.ODNOM,OBDATA_FILES.ODREF " + "from SATELLITE  join OBDATA_FILES on  SATELLITE.ID_SATELLITE = OBDATA_FILES.ID_SATELLITE ";
        this.con.setAutoCommit(false);
        // statement
        PreparedStatement st = this.con.prepareStatement(queryString);
        ResultSet rs = null;
     ...
     * @throws SQLException
     */
    public List<String> getSatellites() throws SQLException
    {
     ...
     * @throws SQLException
     */
    public Map<Integer, String> getSatellitesIdNameMap() throws SQLException
    {
        // map to be returned
    ...
    public void updateObdataFileName(String satName, String path, int obdataType) throws SQLException
    {
        ...
        String substatement = "SELECT s.ID_SATELLITE from SATELLITE s where s.SATELLITE_NAME='" + satName + "' and s.ID_SATELLITE = f.ID_SATELLITE";

        insertStatement = insertStatement + "(" + substatement + ")";

        PreparedStatement st = this.con.prepareStatement(insertStatement);
    ...
    {
        // statement string
        String updateString = "update SATELLITE set ID_ALLOWED_LOOK_SIDE=" + allowedSide + " where SATELLITE_NAME='" + satName + "'";
        this.tm.debug("Inside method updateAllowedLookSide " + updateString);
        // prepared stm
        PreparedStatement st = this.con.prepareStatement(updateString);
